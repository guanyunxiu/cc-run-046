  ...
  actions: {
    ...
    plan(): Promise<PlanResult> {
      ...
      return new Promise((resolve) => {
        ...
        // 注意：Pinia state 是 Vue reactive proxy，无法被 Worker 的
        // structuredClone 克隆（抛 DataCloneError），所有数据都要先拷成普通对象。
        const plain = <T,>(v: T): T => JSON.parse(JSON.stringify(v))
        const req: PlanRequest = {
          type: 'plan',
          terrain: plain(scene.terrain),
          threats: plain(scene.threats),
          noflyZones: plain(scene.noflyZones),
          obstacles: plain(scene.obstacles),
          dynamics: JSON.parse(
            JSON.stringify(scene.dynamics, (k, v) => (v === Infinity ? 1e9 : v))
          ),
          waypoints: plain(scene.waypoints),
          planParams: plain(scene.planParams),
          weights: plain(scene.weights),
          smoothing: this.smoothing
        }
