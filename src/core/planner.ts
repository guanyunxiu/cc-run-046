...
export class GridPlanner {
  ...
  plan(start: Vec3, goal: Vec3): GridPlanResult {
    ...
    while (open.size > 0) {
      ...
      if (curIdx === goalIdx) {
        const gridPath = this.reconstruct(goalIdx)
        return {
          success: true,
          // 在栅格折线两端接上用户指定的精确起终点，保证航迹严格经过
          // 航点位置（栅格中心可能偏离半个到一个栅格）。连接段用用户
          // clearance 口径做净空校验，不满足时退回到栅格中心，绝不穿山。
          path: this.attachExactEndpoints(start, goal, gridPath),
  ...
  /**
   * 将用户指定的精确起终点接到栅格搜索结果两端。
   * 搜索使用的是 clearance + 半栅格对角的膨胀净空，故栅格中心处必满足
   * 用户 clearance；连接段只可能在“精确点本身/直连段”出问题，
   * 逐点与逐段验证后才拼接，保证整段航迹在用户 clearance 口径下可行。
   */
  private attachExactEndpoints(
    start: Vec3,
    goal: Vec3,
    gridPath: Vec3[]
  ): Vec3[] {
    if (gridPath.length === 0) return gridPath
    const clearance = this.params.clearance
    const pts: Vec3[] = []

    const samePoint = (a: Vec3, b: Vec3, eps = 1e-6) =>
      Math.abs(a.x - b.x) <= eps && Math.abs(a.y - b.y) <= eps && Math.abs(a.z - b.z) <= eps

    const first = gridPath[0]
    if (!samePoint(start, first)) {
      if (
        !this.env.isBlocked(start, clearance) &&
        this.env.isSegmentFeasible(start, first, clearance)
      ) {
        pts.push({ ...start })
      }
    }
    pts.push(...gridPath.map((p) => ({ ...p })))

    const last = pts[pts.length - 1]
    if (!samePoint(goal, last)) {
      if (
        !this.env.isBlocked(goal, clearance) &&
        this.env.isSegmentFeasible(last, goal, clearance)
      ) {
        pts.push({ ...goal })
      }
    }
