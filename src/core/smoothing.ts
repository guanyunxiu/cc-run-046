...
/**
 * 三次均匀 B 样条平滑（钳位/clamped 形式）。
 * 首尾控制顶点各复制三次，使曲线严格插值首末点（航点必须被航迹经过）；
 * 内部顶点仍按 B 样条平滑（不要求逐点穿过，这正是平滑的意义）。
 * 输出在 feasibility 不满足时回退折线平滑（端点同样保持不动）。
 */
export function smoothBSpline(
  ...
  // 钳位控制多边形：path[0] 与 path[n-1] 各出现三次（含自身），
  // 三次钳位 B 样条因此严格经过首末点。
  const cp = [
    path[0],
    path[0],
  ...
  const out: Vec3[] = []
  // 跳过由端点三重复制产生的退化首段（前两个控制顶点重合），
  // 从第 2 段开始；曲线仍从 path[0] 出发并以 path[n-1] 结束。
  for (let i = 1; i < cp.length - 3; i++) {
    const p0 = cp[i]
    const p1 = cp[i + 1]
    const p2 = cp[i + 2]
    const p3 = cp[i + 3]
 ...
 * 首尾三重复制端点使曲线严格插值首末点（钳位 Catmull-Rom）；
 * 碰撞不满足时回退折线松弛。
 */
export function smoothBezier(
  ...
  const out: Vec3[] = []
  // 跳过首段退化区间（cp[0..2] 前三个重合），使起点严格为 path[0]
  for (let i = 1; i < cp.length - 3; i++) {
    const p0 = cp[i]
    const p1 = cp[i + 1]
    const p2 = cp[i + 2]
...
export function smoothPolynomial(
  ...
  // 采样参数：均匀采样 + 所有原始节点参数（自然样条在节点处严格插值，
  // 显式包含节点可保证输出点集精确经过各顶点，再由排序/去重统一）。
  const samples = Math.max(path.length * 4, Math.ceil(total / sampleSpacing))
  const us = new Set<number>()
  for (let k = 0; k <= samples; k++) us.add(k / samples)
  for (const uu of u) us.add(uu)
  const usSorted = [...us].sort((a, b) => a - b)

  const out: Vec3[] = []
  let lastP: Vec3 | null = null
  for (const uu of usSorted) {
    const p: Vec3 = { x: 0, y: 0, z: 0 }
    for (const ax of interpAxes) {
      p[ax] = naturalSplineInterp(u, path.map((q) => q[ax]), uu)
    }
    if (lastP && dist(lastP, p) < 1e-6) continue
    out.push(p)
    lastP = p
  }
