 ...
 * anchors 给出不允许移动的精确锚点（逐段平滑时的航点：起点/途经点/终点），
 * 命中的顶点在整个修正过程中钉住不动。
 */
export function repairPathDynamics(
  ...
  const isAnchor = (p: Vec3) =>
    anchors.some(
      (a) => Math.abs(a.x - p.x) < 1e-6 && Math.abs(a.y - p.y) < 1e-6 && Math.abs(a.z - p.z) < 1e-6
    )
  const anchorIdx = new Set<number>()
  if (anchors.length > 0) {
    for (let i = 0; i < pts.length; i++) if (isAnchor(pts[i])) anchorIdx.add(i)
  }
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
      if (anchorIdx.has(i)) continue // 锚点（航点）钉住
      const prev = snap[i - 1]
      const next = snap[i + 1]
